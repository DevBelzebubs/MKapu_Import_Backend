...
export class PermissionCommandService implements IPermissionCommandPort {
  ...
  async registerPermission(dto: RegisterPermissionDto): Promise<PermissionResponseDto> {
  ...
  async updatePermission(dto: UpdatePermissionDto): Promise<PermissionResponseDto> {
    const existingPermission = await this.repository.findById(dto.id_permiso);
    if (!existingPermission) {
      throw new NotFoundException(`Permiso con ID ${dto.id_permiso} no encontrado`);
    }

    if (dto.nombre && dto.nombre !== existingPermission.nombre) {
      const nameExists = await this.repository.existsByName(dto.nombre);
      if (nameExists) {
        throw new ConflictException('El nombre ya está en uso por otro permiso');
      }
    }

    const updatedPermission = PermissionMapper.fromUpdateDto(existingPermission, dto);
    const savedPermission = await this.repository.update(updatedPermission);
    return PermissionMapper.toResponseDto(savedPermission);
  }

  async changePermissionStatus(dto: ChangePermissionStatusDto): Promise<PermissionResponseDto> {
    const existingPermission = await this.repository.findById(dto.id_permiso);
    if (!existingPermission) {
      throw new NotFoundException(`Permiso con ID ${dto.id_permiso} no encontrado`);
    }

    const updatedPermission = PermissionMapper.withStatus(existingPermission, dto.activo);
    const savedPermission = await this.repository.update(updatedPermission);
    return PermissionMapper.toResponseDto(savedPermission);
  }
